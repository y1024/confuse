...
16. [修改方法]，利用重载技术修改函数原型并调用修改形参
    1. 支持‘交换参数’，随机交换参数位置，主要用于多参方法
    2. ‘动态派发’，改变方法调用方式
17. [重命名全局变量]，智能名词替换
18. [修改字体]，对项目中使用的字体随机微调
19. [修改颜色]，对项目中UI控件颜色随机偏移
20. [UI布局偏移]，支持Frame、SnapKit常见布局微调
21. [插入局部变量]，拆分单行复合调用，改变执行顺序
22. [修改局部变量]，模拟人工封装调用，变量名关联类型（支持嵌套），优势：
    1. 局部变量值运行时保持不变，详情见[支持类型汇总表](https://520coding.yuque.com/docs/share/cd70e83b-4ab4-40e5-b719-70f214e869c8)
    2. 可多次执行，指数x2递增
23. [重命名多语言]，支持自定义方法多语言处理
24. [修改字符串]，识别单行、多行、字符串插值、及扩展字符串，改后由加密和拆分字符组等多种方式自由组合，并保留原有字符的注释，方便检查
    1. 设置‘最少长度’过滤
    2. 也可设置‘有效个数’搭配使用
25. [修改xib、storyboard]，自动插入视图，并修改内部结构属性
26. [插入文件]，接近实际开发（结合网络、存储、MVC、xib等，保证代码具有高关联度和实际意义），强化上下文关联。可设置‘文件名前缀’，设置‘**Target**导入’方式和OC相同
27. [插入文本]，生成json、txt、plist等常用文本文件，项目中自动高关联调用（引入属性、初始化、自动销毁等）。**注意：**(在项目根路径下，会生成"**other_xxx_text**"的文件夹，生成的文件会**自动导入**）
28. [重命名类]，类名不限制（例如：my、My），识别嵌套类型及typealias，支持class、struct、enum、protocol
    1. 可设置‘重命名同名文件’
    2. 可设置‘前缀’
